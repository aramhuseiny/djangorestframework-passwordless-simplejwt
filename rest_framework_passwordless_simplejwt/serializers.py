...
from . import CustomJWTAuthentication
...
class TokenObtainSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        authenticator = CustomJWTAuthentication.JWTAuthentication()
        # authenticator = authentication.JWTAuthentication()
        self.user, *obj = authenticator.authenticate(self.context["request"])
...
class TokenVerifySerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if (
            ...
            and "rest_framework_passwordless_simplejwt.token_blacklist" in settings.INSTALLED_APPS
        ):
...
class SendCodeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        self.user.otp = random.randint(111111, 999999)
        self.user.otp_expiration = datetime.now() + timedelta(minutes = 5)
        self.user.save()
        try:
            ...
            if req.status_code == 200:
                return {
                    "message": f"Code sent to {self.user.mobile}"
                }
            else:
                return {
