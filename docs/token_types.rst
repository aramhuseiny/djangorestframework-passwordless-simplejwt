...
``'rest_framework_passwordless_simplejwt.tokens.AccessToken'`` dot path by default but may
also include the ``'rest_framework_passwordless_simplejwt.tokens.SlidingToken'`` dot path.
...
``('rest_framework_passwordless_simplejwt.tokens.SlidingToken',)``.  (Alternatively, the
``AUTH_TOKEN_CLASSES`` setting may include dot paths to both the
``AccessToken`` and ``SlidingToken`` token classes in the
``rest_framework_passwordless_simplejwt.tokens`` module if you want to allow both token
...
.. code-block:: python
  ...
  from rest_framework_passwordless_simplejwt.views import (
